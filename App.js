...
export default function RegaloSorpresa() {
  const opcionesBase = [
    ...
    { id: 6, titulo: 'Un escape room', descripcion: 'Perfecto para poner a prueba vuestra inteligencia... aunque no será necesario, porque no es este 😅' },
  ...
  useEffect(() => {
    // Barajar las opciones al cargar
    const mezcladas = [...opcionesBase].sort(() => Math.random() - 0.5);
    setOpciones(mezcladas);
  }, []);

  // Fecha de activación: 9 noviembre 2025, 15:00 hora UK
    ...
    <div className="min-h-screen flex flex-col items-center justify-center bg-gradient-to-br from-emerald-100 to-emerald-300 p-4 text-center">
      {!seleccion ? (
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="max-w-md w-full">
          ...
          {contador > 0 && (
            <p className="mt-4 text-sm text-gray-700">Vaya, pues resulta que tampoco era ese el regalo. Pero no os preocupéis, podréis elegir otra opción pronto, exactamente en {formatoTiempo(contador)}. Podéis verlo como una especie de calendario de adviento digital-bodil. Una idea genial, ¿no os parece? A ver si no tardáis mucho en acertar la opción correcta... ¡Buena suerte!</p>
          )}
        </motion.div>
      ) : (
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="max-w-md w-full">
